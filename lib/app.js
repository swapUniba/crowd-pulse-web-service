...
var scheduleJobSetup = function () {
  var timeout = config.batch.timeout;
  if (timeout || timeout === '') {
    timeout = '*/2 * * * *';    // TODO change default value
  }
  schedule.scheduleJob(timeout, function(){
    batch.updateUserSocialInformation(true, true, true);
    batch.executeCrowdPulseStandardPipeline();
  });
};
