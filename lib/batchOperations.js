...
var updateInterestsForUser = function (username) {
  ...
  return dbConnection.connect(config.database.url, username).then(function (conn) {
    return conn.Message.find({}, function (err, messages) {
      var timestamp = new Date().getTime();
            ...
            message.tokens.forEach(function (token) {
              if (!token.stopWord) {
                if (token.text.startsWith('#')) {
                  interests.push({
                    value: token.text.substr(1),    // TODO remove hashtag ?
      ...
      // copy likes
      conn.Like.find({},  function (err, likes) {
        if (!err) {
        ...
        // copy app category
        conn.PersonalData.find({source: 'appinfo'},  function (err, apps) {
          if (!err) {
