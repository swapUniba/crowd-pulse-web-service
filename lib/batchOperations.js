...
exports.updateUserSocialInformation = function (profiles, messages, likes, friends) {
  ...
  // TODO performs batch operation for global data
          ...
          setTimeout(function () {
            ...
            if (profile.identities.configs.linkedInConfig.linkedInId) {
              ...
              if (profile.identities.configs.fitbitConfig.fitbitId) {
                if (profiles) {
                  fitbit.updateUserProfile(profile.username, null);
                  fitbit.updateDailyFriends(profile.username, null);
                  fitbit.updateDailyFood(profile.username, null);
                  fitbit.updateDailySleep(profile.username, null);
                  fitbit.updateDailyActivity(profile.username, null);
                  fitbit.updateDailyHeartRate(profile.username, null);
                  fitbit.updateDailyWeight(profile.username, null);
                  fitbit.updateDailyFat(profile.username, null);
                  fitbit.updateDailyBmi(profile.username, null);
...
exports.cleanPersonalData = function() {
  ...
  var dbConnection = new CrowdPulse();
  dbConnection.connect(config.database.url, databaseName.globalData).then(function (conn) {

    // clean GPS global data
    return conn.PersonalData.deleteMany({source: 'gps', latitude: '0', longitude: '0'}, function (err) {
      if (err) {
        console.log(err);
      } else {
        console.log("GPS inconsistent data deleted from " + databaseName.globalData + " at " + new Date());
      }
      dbConnection.disconnect();
    });
  });
    ...
    return conn.Profile.find(function (err, profiles) {
      ...
      if (profiles) {
        profiles.forEach(function (profile) {
          ...
          // TODO add here other cleaning operations (eg. duplicates data, null data, etc)
        });
      }
